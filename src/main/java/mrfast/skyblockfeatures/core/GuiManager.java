...
public class GuiManager {
    ...
    public void renderPlayerInfo(final RenderGameOverlayEvent.Post event) {
        if (skyblockfeatures.usingLabymod && !(Minecraft.getMinecraft().ingameGUI instanceof GuiIngameForge)) return;
        if (event.type != RenderGameOverlayEvent.ElementType.EXPERIENCE && event.type != RenderGameOverlayEvent.ElementType.JUMPBAR) return;
        if (Minecraft.getMinecraft().currentScreen instanceof LocationEditGui || Minecraft.getMinecraft().theWorld == null) return;
    ...
    public void renderPlayerInfoLabyMod(final RenderGameOverlayEvent event) {
        if (!skyblockfeatures.usingLabymod) return;
