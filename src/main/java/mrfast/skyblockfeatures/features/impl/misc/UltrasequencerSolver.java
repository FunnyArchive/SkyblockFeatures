    ...
    public void onSlotClick(SlotClickEvent event) {
        ...
        if (Utils.GetMC().currentScreen instanceof GuiChest) {
            ...
            if (chestName.startsWith("Ultrasequencer (")) {
                ...
                try {
                    int slotInt = Integer.parseInt(Utils.cleanColour(event.slot.getStack().getDisplayName()).replaceAll("[^0-9]", ""));
                    ...
                    for (Slot slot : invSlots) {
                        if(answers.containsKey(slot)) {
                            if(slot.getStack()!=null) {
                                try {
                                    int slotInt = Integer.parseInt(Utils.cleanColour(slot.getStack().getDisplayName()).replaceAll("[^0-9]", ""));
