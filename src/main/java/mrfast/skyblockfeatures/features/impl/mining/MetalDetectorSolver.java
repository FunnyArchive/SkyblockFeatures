        ...
        if (event.type == 2) {
            ...
            for(String segment:actionBarSplit) {
                if(nextSegmentIsDistance) {
                    if(segment.contains(".")) distanceToTreasure = Double.parseDouble(Utils.cleanColour(segment).replaceAll("[^0-9]", ""))/10;
                    else distanceToTreasure = Double.parseDouble(Utils.cleanColour(segment).replaceAll("[^0-9]", ""));
