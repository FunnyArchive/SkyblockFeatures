...
public class IcePathSolver {
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        if (event.phase != TickEvent.Phase.START || !Utils.inDungeons || mc.thePlayer == null || mc.theWorld == null || !skyblockfeatures.config.IcePathSolver) return;
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        if (skyblockfeatures.config.IcePathSolver && silverfishChestPos != null && roomFacing != null && grid != null && IcePathSolver.silverfish.isEntityAlive()) {
            for (int i = 0; i < steps.size() - 1; i++) {
                ...
                RenderUtil.draw3DLine(pos.addVector(0.5, 0.5, 0.5), pos2.addVector(0.5, 0.5, 0.5), 5, Color.yellow, event.partialTicks);
                GlStateManager.enableCull();
            }
            RenderUtil.drawOutlinedFilledBoundingBox(IcePathSolver.silverfish.getEntityBoundingBox(), Color.yellow, event.partialTicks);
        }
    }
