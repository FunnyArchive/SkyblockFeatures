...
public class AuctionPriceOverlay {
    ...
    public void onGuiOpen(GuiOpenEvent event) {
        if (!Utils.inSkyblock || !skyblockfeatures.config.betterAuctionPriceInput) return;
    ...
    public void onKeyInput(GuiScreenEvent.KeyboardInputEvent keyboardInputEvent) {
        ...
        if (screen instanceof GuiChest && Keyboard.isKeyDown(skyblockfeatures.reloadAH.getKeyCode())){
            ContainerChest ch = (ContainerChest) ((GuiChest)screen).inventorySlots;
            if (!ch.getLowerChestInventory().getName().contains("Auction") || ready) return;
            ready = true;
            Utils.setTimeout(()->{
                Utils.GetMC().playerController.windowClick(Utils.GetMC().thePlayer.openContainer.windowId, 53, 0, 0, Utils.GetMC().thePlayer);
                Utils.setTimeout(()->{
                    ready = false;
